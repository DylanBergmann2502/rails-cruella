...
`.envs` directories are hidden from AI agents by default — verify their location with `ls`. Docker Compose files use relative paths: `env_file: - ./.envs/.rails` (relative to the compose file at `deploy/local/web.yml`). Build context is `../..` (project root).
